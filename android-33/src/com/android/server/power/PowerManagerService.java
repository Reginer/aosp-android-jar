...
public final class PowerManagerService extends SystemService
    ...
    // Message: 保持最高亮度超时
    private static final int MSG_SCREEN_TOP_TIMEOUT = 1992;
    ...
    private void updateSettingsLocked() {
        ...
        mScreenLightValue = Settings.System.getIntForUser(mContext.getContentResolver(), Settings.System.SCREEN_BRIGHTNESS, 0, UserHandle.USER_CURRENT);
    ...
    private void userActivityInternal(int displayId, long eventTime, int event, int flags,
                                      int uid) {
        synchronized (mLock) {
            mHandler.removeMessages(MSG_SCREEN_TOP_TIMEOUT);
            final Message msg = mHandler.obtainMessage(MSG_SCREEN_TOP_TIMEOUT);
            msg.setAsynchronous(true);
        ...
        public boolean handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_ATTENTIVE_TIMEOUT:
                    ...
                    break;
                case MSG_SCREEN_TOP_TIMEOUT:
                    handleChangeScreenLight();
                    break;
            }
    ...
    private static final int SCREEN_LIGHT_VALUE_60_PERCENT = 33;
    private boolean modify8Me = false;

    /**
     * 判断当前亮度，低于60不处理，高于60降低亮度
     */
    private void handleChangeScreenLight() { // runs on handler thread
        if (mScreenLightValue > SCREEN_LIGHT_VALUE_60_PERCENT) {
            changeBrightness(SCREEN_LIGHT_VALUE_60_PERCENT);
        }
    }

    private void changeBrightness(int lightValue) {
        final long ident = Binder.clearCallingIdentity();
        try {
            File file = new File("/sys/class/leds/lcd-backlight/brightness");
            FileWriter fileWriter = new FileWriter(file, false);
            fileWriter.write(String.valueOf(lightValue));
            fileWriter.flush();
            fileWriter.close();
            modify8Me = true;
        } catch (Exception e) {
     ...
     * 发送亮度持续时间超时消息
     */
    private void sendChangeScreenMessage() { // runs on handler thread
        mHandler.removeMessages(MSG_SCREEN_TOP_TIMEOUT);
        final Message msg = mHandler.obtainMessage(MSG_SCREEN_TOP_TIMEOUT);
        msg.setAsynchronous(true);
        mHandler.sendMessageDelayed(msg, 20 * 60 * 1000);
        if (modify8Me) {
            if (mScreenLightValue > SCREEN_LIGHT_VALUE_60_PERCENT) {
                changeBrightness(mScreenLightValue);
            }
            modify8Me = false;
        }
    }
