...
class ByteBufferAsIntBuffer                  // package-private
    extends IntBuffer
{
    ...
    */
    ...
    private void putUnchecked(int index, int[] src, int off, int length) {
    ...
    void putBuffer(int pos, IntBuffer src, int srcPos, int n) {
        ...
        if (order() == src.order() &&
              src instanceof ByteBufferAsIntBuffer asSrc) { // always true if src.hb == null
            this.bb.putBuffer(ix(pos), asSrc.bb, asSrc.ix(srcPos), n << 2);
            return;
        }
